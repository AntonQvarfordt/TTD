...
//				Undo.RegisterUndo("Bajs", );
	...
	void DrawPointInspector(BezierPoint point, int index)
	{
		...
		{
			Undo.RegisterCompleteObjectUndo(point.transform, "Remove Point");
	...
	{	
		...
		var fmh_197_64_637896525876644424 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position)*0.1f, Vector3.zero, Handles.RectangleHandleCap);
		...
		if(point.handleStyle != BezierPoint.HandleStyle.None)
		{
			...
			var fmh_208_69_637896525876648657 = point.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(point.globalHandle1, HandleUtility.GetHandleSize(point.globalHandle1)*0.075f, Vector3.zero, Handles.CircleHandleCap);
			if(point.globalHandle1 != newGlobal1)
			{
				Undo.RegisterCompleteObjectUndo(point.transform, "Move Handle");
			...
			var fmh_216_69_637896525876652489 = point.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(point.globalHandle2, HandleUtility.GetHandleSize(point.globalHandle2)*0.075f, Vector3.zero, Handles.CircleHandleCap);
