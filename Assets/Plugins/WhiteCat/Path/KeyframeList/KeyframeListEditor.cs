...
namespace WhiteCat.Paths
{
	public partial class Path
	{
		...
		{
			// 编辑器个性化颜色
			public Color personalizedColor = EditorUI.GetRandomColor();
			...
			protected sealed override void Editor_OnInspectorGUI()
			{
				...
				if (GUI.Button(rect2, GUIContent.none))
				{
					...
					personalizedColor = EditorUI.GetRandomColor();
				...
				EditorUI.BeginGUIColor(personalizedColor);
				...
				bool edit = GUI.Toggle(_rect, _currentEditing == this, EditorAssets.editTexture, EditorUI.buttonStyle);
				...
				if (!isSorted)
				{
					if (EditorUI.IndentedButton("Sort"))
					{
		...
		public partial class KeyframeList<KeyValue, Keyframe, Component>
		{
			...
			protected sealed override void DrawToolBar(Rect rect)
			{
				...
				_showKeyDetail = GUI.Toggle(rect, _showKeyDetail, globalGUIContent, EditorUI.buttonStyle);

				// Position

				EditorGUI.BeginChangeCheck();

				EditorUI.BeginLabelWidth(_distanceLabelWidth);
				...
				if (GUI.Button(rect, globalGUIContent, EditorUI.buttonLeftStyle))
				{
				...
				if (GUI.Button(rect, globalGUIContent, EditorUI.buttonRightStyle))
				{
					Undo.RecordObject(this, "Insert Key");
					InsertForward();
					EditorUtility.SetDirty(this);
				...
				if (GUI.Button(rect, globalGUIContent, EditorUI.buttonStyle))
				{
